...
INSERT INTO RESERVATION (reservenumber, selectseat, moviecode, screencode, theatercode, birth, phone, pw, usercheck) VALUES ('1234-5678-91','F01;F02','M001','S002','T001','2000-05-01','010-8251-6374','1234','O');
INSERT INTO RESERVATION (reservenumber, selectseat, moviecode, screencode, theatercode, birth, phone, pw, usercheck) VALUES ('2345-6789-10','E05;E06','M002','S001','T002','1995-11-30','010-1111-2222','1234','X');
select THEATER.*, MOVIECENTER.* from THEATER inner join MOVIECENTER on THEATER.centercode = MOVIECENTER.centercode where theater.centercode = 'C001';
select MOVIE.*, SCREEN.* from MOVIE inner join SCREEN on MOVIE.moviecode = SCREEN.moviecode where MOVIE.moviecode = 'M001';
select THEATER.*, SCREEN.* from THEATER inner join SCREEN on THEATER.theatercode = SCREEN.theatercode where SCREEN.theatercode = 'T001';
select RESERVATION.*, THEATER.* from RESERVATION inner join THEATER on RESERVATION.theatercode = THEATER.theatercode where THEATER.theatercode = 'T001';
select RESERVATION.*, SCREEN.* from RESERVATION inner join SCREEN on RESERVATION.screencode = SCREEN.screencode where RESERVATION.screencode = 'S001';
select RESERVATION.*, MOVIE.* from RESERVATION inner join MOVIE on RESERVATION.moviecode = MOVIE.moviecode where RESERVATION.moviecode = 'M001';